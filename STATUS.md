...
| Verdict stability | **Measured** | `GET /v1/eval/stability`. Repeating that pass showed only 1 of 5 cases giving the same verdict every time, and 5 false-confident identifications across 19 runs where the single pass had none. D04 returned the same wrong film in 3 of 4 runs. `probable` was never reached in any run. Two defects were found and fixed: the candidate slot accepted non-films (`app/works.py`), and hypotheses were never opposed (two Heuer thresholds in the gate). |
