...
So the question we built for was not "can a model identify this film". Almost every AI answer here is a screenshot matcher, which works only when the answer is already in a reference set — precisely the case that does not apply to unidentified material. And the dangerous failure is not a wrong answer, it is a **plausible** one: a catalogue entry that looks researched, propagates for thirty years, and quietly misattributes a film.
